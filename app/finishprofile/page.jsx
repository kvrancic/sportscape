...
import { TextInput, Rating, FileInput, Radio, Group, Stack, NumberInput, Button, Space, Textarea, Text, Container, Title, SimpleGrid } from '@mantine/core';
...
export default function PrivatePage() {
  const form = useForm({
    ...
    validate: {
      ...
      basketballSkill: (value, values) => (values.accountType === 'athlete' && (!value || value < 1 || value > 5) ? 'Skill level must be between 1 and 5' : null),
      footballSkill: (value, values) => (values.accountType === 'athlete' && (!value || value < 1 || value > 5) ? 'Skill level must be between 1 and 5' : null),
      volleyballSkill: (value, values) => (values.accountType === 'athlete' && (!value || value < 1 || value > 5) ? 'Skill level must be between 1 and 5' : null),
    },
  });
  ...
  return (
    <Container className="flex flex-col items-center justify-center py-12">
      <div className="w-full max-w-2xl p-6 rounded-lg shadow-2xl">
        ...
        <form onSubmit={form.onSubmit(handleSubmit)} className="flex flex-col space-y-4">
              ...
              <SimpleGrid cols={1} spacing="md">
                <div className="flex flex-col items-center">
                  ...
                  {form.errors.basketballSkill && (
                    <Text c="red" size="xs">{form.errors.basketballSkill}</Text>
                  )}
                </div>
                <div className="flex flex-col items-center">
                  ...
                  {form.errors.footballSkill && (
                    <Text c="red" size="xs">{form.errors.footballSkill}</Text>
                  )}
                </div>
                <div className="flex flex-col items-center">
                  ...
                  {form.errors.volleyballSkill && (
                    <Text c="red" size="xs">{form.errors.volleyballSkill}</Text>
                  )}
                </div>
              </SimpleGrid>
            </div>
          ...
          <Button type="submit" className="bg-orange-500 hover:bg-orange-600 focus:ring-orange-500 focus:ring-offset-orange-200">
            Finish Setup
          </Button>
        </form>
      </div>
